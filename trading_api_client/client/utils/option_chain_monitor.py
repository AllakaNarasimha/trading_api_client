...
class OptionChainMonitor:
        ...
        while not self.health_check_stop_event.is_set():
            try:
                ...
                if not is_running and not thread_alive and saved_config:
                    ...
                    logger.debug(f"[HEALTH CHECK] fetch_monitor thread is running (ID: {self.monitor_thread.ident})")
    ...
    def _ensure_price_watcher_initialized(self) -> None:
        """
        ...
        if self.price_watcher is None:
            try:
                self.price_watcher = PriceWatcher()
                logger.debug("Initialized price watcher")
            except Exception as e:
                logger.error(f"Failed to initialize price watcher: {e}")
        
    def fetch_historical_data(self) -> None:
        """
        Fetch historical data for configured symbol if enabled.
        Can be called separately from polling operations.
        Ensures watchers are initialized before fetching.
        """
        # Ensure watchers are initialized before fetching
        self._ensure_price_watcher_initialized()
        
        if not self.price_watcher:
            logger.warning("No watchers available for historical data fetch")
            return
        
        historical_fetcher = HistoricalDataFetcher(self.price_watcher)
    ...
    def start_live_price_monitoring(self) -> None:
        ...
        try:
            ...
            self.live_price_fetcher = LivePriceFetcher(self.price_watcher)
    ...
    def stop_live_price_monitoring(self) -> None:
        ...
        try:
            if not self.live_price_fetcher:
                logger.debug("Live price fetcher not initialized")
                return
    ...
    def poll_option_chain(self, symbol_config: Dict[str, Dict[str, Any]]) -> None:
        ...
        is_market_open = True
        if self.price_watcher is not None:
            is_market_open = self.price_watcher.is_market_open()   
        ...
        try:
            ...
            if csv_export_enabled:
                ...
                for sym in self.current_symbols:
                    # Note: Using 'OPTION_CHAIN' as data_type to distinguish from LIVE/HISTORICAL
                    # FileUtils will create the appropriate folder structure
                    symbol_filenames[sym] = FileUtils.get_csv_file_path(sym, data_type='OPTION_CHAIN')
                    ...
                    if symbols_to_poll:
                        ...
                        for symbol in symbols_to_poll:
                            try:
                                ...
                                option_chain = self.price_watcher.get_option_chain(symbol, symbol_strikes)
