...
def sync_to_minute_boundary(stop_event: threading.Event) -> None:
    """
...
def sync_to_second_boundary(stop_event: threading.Event) -> None:
    """
    Synchronize to the next second boundary.
    Waits until the current microsecond portion of the second is complete.
    ...
    current_time = datetime.datetime.now()
    microseconds_into_second = current_time.microsecond
    sleep_time = (1000000 - microseconds_into_second) / 1000000.0  # Convert to seconds
    if sleep_time > 0.001:  # Only sleep if there's meaningful time left
...
class OptionChainMonitor:
    ...
    def _cleanup_resources(self) -> None:
        """Clean up resources (watchers, managers, database connections)."""
    ...
    def poll_option_chain(self, symbol_config: Dict[str, Dict[str, Any]]) -> None:
        ...
        try:
            ...
            sync_to_minute_boundary(self.stop_event)
        ...
        try:
            ...
            with self.state_lock:
                ...
                try:
                    ...
                    symbol_name = new_symbol.split(':')[-1] if ':' in new_symbol else new_symbol
