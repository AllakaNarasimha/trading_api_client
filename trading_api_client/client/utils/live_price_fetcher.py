...
class LivePriceFetcher:
    ...
    def __init__(self, price_watcher: PriceWatcher):
        """
        Initialize Live Price Fetcher.
        
        Args:
            price_watcher: PriceWatcher instance for managing price data
        """
    ...
    def insert_live_price_to_db(self, data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            if not data or 'symbol' not in data:
                logger.warning(f"No data to insert for {data.get('symbol', 'unknown')}")
                return False           
            

            # Check if HistoryDataManager is available
            if self.sql_data_manager is None:
                logger.error(f"HistoryDataManager not available for {data.get('symbol', 'unknown')}")
                return False
            ...
            if config.LIVE_DATA_CSV_EXPORT_ENABLED:
                self._save_live_price_to_csv(data)
    ...
    def _save_live_price_to_csv(self, data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            success = FileUtils.append_dict_to_csv(data, file_path)
            if not success:
                logger.warning(f"Failed to save live price to CSV for {symbol}")
    ...
    def _convert_live_price_message(self, symbol: str, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            converted_data = {
                'timestamp': message.get('exch_feed_time', timestamp),
    ...
    def _live_price_callback(self, symbol: str, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if config.LIVE_DATA_ENABLED:
                self.insert_live_price_to_db(converted_data)
