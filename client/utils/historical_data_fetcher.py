...
class HistoricalDataFetcher:
    def __init__(self):
        self.priceWatcher = PriceWatcher('history')
    ...
    def insert_historical_data_to_db(self, data: list, symbol: str) -> bool:
        ...
        try:
            ...
            insert_status = dmg.insert_historical_data(data)
            logger.debug(f"Logged live price to database for {symbol} - Status: {insert_status}")
    ...
    def fetch_historical_data(self) -> None:
        ...
        for index, historical_symbol in enumerate(self.symbols):
            try:
                self._fetch_historical_data_for_symbol(historical_symbol, historical_backdays, historical_duration)
                logger.info(f"Index: {index} Completed historical data fetch for symbols: {self.symbols}")
            except Exception as e:
                logger.error(f"Index: {index} Error fetching historical data for symbol {historical_symbol}: {e}")
        logger.info(f"<-- Completed historical data fetch for {len(self.symbols)} symbols: {self.symbols}")
    ...
    def _fetch_historical_data_for_symbol(self, historical_symbol: str, historical_backdays: int, 
        ...
        try:
            # Calculate timestamps for historical data (past historical_backdays, from 9:00 AM to 3:30 PM)
            today = datetime.date.today()
            end_time = datetime.datetime.combine(today, datetime.time(15, 30))
            start_date = today - datetime.timedelta(days=historical_backdays)
            ...
            historical_data = self.priceWatcher.get_historical_data(
            ...
            logger.info(f"Fetched historical data for {historical_symbol}, and data {historical_data}") 
