...
class Config:
    ...
    def _load_config(self) -> None:
        ...
        # Retry settings
        self.RETRY_CUTOFF_TIME = "15:40"
        self.RETRY_MARKET_OPEN_TIME = "09:15"
        self.RETRY_MARKET_CLOSE_TIME = "15:30"
        self.RETRY_INITIAL_RETRY_COUNT = 3
        self.RETRY_INITIAL_RETRY_DELAYS = [1, 2, 4]
        self.RETRY_PERSISTENT_RETRY_INTERVAL = 30.0  # Default 30 seconds
        self.RETRY_IDLE_TIMEOUT = 300.0  # Default 5 minutes (300 seconds) - restart if no key updates
        self.RETRY_ENABLE_LOGGING = True
        self.RETRY_LOGS_DIR = "logs"
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                # Load retry settings
                retry_section = root.find('retry')
                if retry_section is not None:
                    if retry_section.find('cutoff_time') is not None:
                        self.RETRY_CUTOFF_TIME = retry_section.find('cutoff_time').text or self.RETRY_CUTOFF_TIME
                    if retry_section.find('market_open_time') is not None:
                        self.RETRY_MARKET_OPEN_TIME = retry_section.find('market_open_time').text or self.RETRY_MARKET_OPEN_TIME
                    if retry_section.find('market_close_time') is not None:
                        self.RETRY_MARKET_CLOSE_TIME = retry_section.find('market_close_time').text or self.RETRY_MARKET_CLOSE_TIME
                    if retry_section.find('initial_retry_count') is not None:
                        self.RETRY_INITIAL_RETRY_COUNT = int(retry_section.find('initial_retry_count').text or self.RETRY_INITIAL_RETRY_COUNT)
                    if retry_section.find('initial_retry_delays') is not None:
                        delays_text = retry_section.find('initial_retry_delays').text or '1,2,4'
                        self.RETRY_INITIAL_RETRY_DELAYS = [int(s.strip()) for s in delays_text.split(',')]
                    if retry_section.find('persistent_retry_interval') is not None:
                        self.RETRY_PERSISTENT_RETRY_INTERVAL = float(retry_section.find('persistent_retry_interval').text or self.RETRY_PERSISTENT_RETRY_INTERVAL)
                    if retry_section.find('idle_timeout') is not None:
                        self.RETRY_IDLE_TIMEOUT = float(retry_section.find('idle_timeout').text or self.RETRY_IDLE_TIMEOUT)
                    if retry_section.find('enable_retry_logging') is not None:
                        self.RETRY_ENABLE_LOGGING = retry_section.find('enable_retry_logging').text.lower() == 'true'
                    if retry_section.find('logs_dir') is not None:
                        self.RETRY_LOGS_DIR = retry_section.find('logs_dir').text or self.RETRY_LOGS_DIR
        ...
        # Retry settings environment overrides
        if os.getenv('RETRY_CUTOFF_TIME'):
            self.RETRY_CUTOFF_TIME = os.getenv('RETRY_CUTOFF_TIME')
        if os.getenv('RETRY_MARKET_OPEN_TIME'):
            self.RETRY_MARKET_OPEN_TIME = os.getenv('RETRY_MARKET_OPEN_TIME')
        if os.getenv('RETRY_MARKET_CLOSE_TIME'):
            self.RETRY_MARKET_CLOSE_TIME = os.getenv('RETRY_MARKET_CLOSE_TIME')
        if os.getenv('RETRY_INITIAL_RETRY_COUNT'):
            self.RETRY_INITIAL_RETRY_COUNT = int(os.getenv('RETRY_INITIAL_RETRY_COUNT'))
        if os.getenv('RETRY_INITIAL_RETRY_DELAYS'):
            self.RETRY_INITIAL_RETRY_DELAYS = [int(s.strip()) for s in os.getenv('RETRY_INITIAL_RETRY_DELAYS').split(',')]
        if os.getenv('RETRY_PERSISTENT_RETRY_INTERVAL'):
            self.RETRY_PERSISTENT_RETRY_INTERVAL = float(os.getenv('RETRY_PERSISTENT_RETRY_INTERVAL'))
        if os.getenv('RETRY_IDLE_TIMEOUT'):
            self.RETRY_IDLE_TIMEOUT = float(os.getenv('RETRY_IDLE_TIMEOUT'))
        if os.getenv('RETRY_ENABLE_LOGGING'):
            self.RETRY_ENABLE_LOGGING = os.getenv('RETRY_ENABLE_LOGGING').lower() == 'true'
        if os.getenv('RETRY_LOGS_DIR'):
            self.RETRY_LOGS_DIR = os.getenv('RETRY_LOGS_DIR')
    ...
    def get_retry_settings(self) -> Dict[str, Any]:
        """Get retry-related settings."""
        return {
            'cutoff_time': self.RETRY_CUTOFF_TIME,
            'market_open_time': self.RETRY_MARKET_OPEN_TIME,
            'market_close_time': self.RETRY_MARKET_CLOSE_TIME,
            'initial_retry_count': self.RETRY_INITIAL_RETRY_COUNT,
            'initial_retry_delays': self.RETRY_INITIAL_RETRY_DELAYS,
            'persistent_retry_interval': self.RETRY_PERSISTENT_RETRY_INTERVAL,
            'idle_timeout': self.RETRY_IDLE_TIMEOUT,
            'enable_logging': self.RETRY_ENABLE_LOGGING,
            'logs_dir': self.RETRY_LOGS_DIR
        }