        ...
        for symbol in self.current_symbols:
            if symbol not in self.current_watchers:
                try:
                    ...
                    logger.debug(f"Initialized watcher for symbol: {symbol}")
                except Exception as e:
                    logger.error(f"Failed to initialize watcher for {symbol}: {e}")
            else:
                logger.debug(f"Watcher for {symbol} already exists, skipping initialization")
    ...
    def add_symbol(self, new_symbol: str, poll_secs: Optional[List[int]] = None, 
        ...
        try:
            ...
            with self.state_lock:
                ...
                try:
                    self.current_watchers[new_symbol] = PriceWatcher()
