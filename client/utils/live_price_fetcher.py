...
class LivePriceFetcher:
    ...
    def __init__(self, watchers: Dict[str, PriceWatcher]):
        ...
        if config.LIVE_DATA_SYMBOLS:
            ...
            logger.info(f"LivePriceFetcher using watcher symbols: {self.symbols}")
    ...
    def subscribe_to_live_prices(self) -> None:
        ...
        for symbol in self.symbols:
            try:
                ...
                if not self.watchers[symbol]:
                    logger.warning(f"No watcher instance for symbol {symbol}")
                    continue
                # Subscribe to live prices
                success = self.watchers[symbol].subscribe_to_price(
    ...
    def unsubscribe_from_live_prices(self) -> None:
        ...
        for symbol in list(self.subscribed_symbols):
            try:
                ...
                success = self.watchers[symbol].unsubscribe_from_price(symbol)
    ...
    def subscribe_with_custom_callback(self, symbol: str, callback: Callable[[str, Dict[str, Any]], None]) -> bool:
        ...
        try:
            ...
            success = self.watchers[symbol].subscribe_to_price(symbol, callback)
