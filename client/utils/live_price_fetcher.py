...
class LivePriceFetcher:
    ...
    def __init__(self, watchdog=None):
        ...
        Args:
            watchdog: Watchdog instance for monitoring activity
        """
    ...
    def insert_live_price_to_db(self, data: Dict[str, Any], symbol: str) -> bool:
        ...
        try:
            ...
            insert_status = dmg.insert_historical_data([data], 'live_data')
            logger.info(f"Logged live price to database for {symbol} - Status: {insert_status}")
    ...
    def _live_price_callback(self, symbol: str, message: Dict[str, Any]) -> None:
        ...
        try:
            if self.watchdog:
                self.watchdog.update_key(f"live_price_{symbol}")
    ...
    def subscribe_to_live_prices(self) -> None:
        ...
        try:
            ...
            if not self.price_watcher:
                logger.warning("Price watcher not available")
                return
            
            # Check if all symbols are already subscribed
            if all(symbol in self.subscribed_symbols for symbol in self.symbols):
            ...
            success = self.price_watcher.subscribe_to_price(
                self.symbols, self._live_price_callback           
            )
            
            if success:
                for symbol in self.symbols:
                    self.subscribed_symbols.add(symbol)
                logger.info(f"Subscribed to live prices for {self.symbols}")
            else:
                logger.warning(f"Failed to subscribe to live prices for {self.symbols}")
