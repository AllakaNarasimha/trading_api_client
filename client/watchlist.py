...
class PriceWatcher:
    ...
    def add_to_watchlist(self, symbol):
        ...
        self.subscribe_to_price(self.watchlist, self.ltp_callback)
    ...
    def subscribe_to_price(self, symbols: Union[str, List[str]], ltp_callback) -> bool:
        """Subscribe to real-time LTP price updates for symbols."""
        try:
            if not self.price_watcher:
                self.logger.warning(f"Price watcher not available for {symbols}")
                return False
            
            if isinstance(symbols, str):
                symbols = [symbols]
            
            return self.price_watcher.subscribe_to_price(symbols, ltp_callback or self.ltp_callback)
        except Exception as e:
    ...
    def subscribe_watchlist_to_price(self, callback: Callable) -> bool:
        ...
        success = True
        if not self.price_watcher.subscribe_to_price(self.watchlist, callback or self.ltp_callback):
            success = False
        return success
